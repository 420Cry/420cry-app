...
import { GET } from 'src/app/api/indicator/fear-and-greed-historical/route'
...
describe('GET /api/coin-market-cap/fear-and-greed-historical', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('returns 200 with data when API responds successfully', async () => {
    const fakeData = { dataPoint: 123 }
    ;(RequestService.axiosGet as any).mockResolvedValue({
      status: 200,
      data: { fear_and_greed_historical: fakeData },
    })
