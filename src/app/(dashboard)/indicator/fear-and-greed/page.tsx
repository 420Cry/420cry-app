...
export default function FearAndGreedPage(): JSX.Element {
  const [indexData, setIndexData] = useState<IFearAndGreedIndexData | null>(
    null,
  )
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold mb-4">Fear and Greed Index</h1>
      ...
      >
        ...
        <text
          ...
          fill="#333"
        >
          {indexData ? indexData.data.value : '...'}
        </text>
        ...
        <text
          ...
          fill="#666"
        >
          {indexData ? indexData.data.value_classification : ''}
        </text>

        {/* Needle / Circle Pointer */}
        {indexData &&
          (() => {
            const value = indexData.data.value // already a number
            const baseRadius = 70
            const offset = 15 // lift the circle slightly above the arc
